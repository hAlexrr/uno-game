...
export default function GameBoard({
    ...
    const getPlayerPosition = (index: number, totalPlayers: number) => {
        ...
        const radius = Math.min(150, Math.min(window.innerWidth, window.innerHeight) * 0.25) // Adjust radius based on screen size
        ...
        <div className="h-full w-full relative flex items-center justify-center">
            ...
            {players.map((player, index) => {
                ...
                return (
                    ...
                    >
                        <div className="text-sm font-medium text-center whitespace-nowrap bg-black/50 px-2 py-1 rounded-lg">
                            {player.name}
                        ...
                        <div className="flex gap-1 max-w-[120px] overflow-x-auto">
                            {Array(player.cards.length).fill(0).map((_, i) => (
                                <div
                                    key={i}
                                    className="w-4 h-6 rounded-sm bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 flex-shrink-0"
                                />
                            ))}
                        </div>
                    </motion.div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="w-20 h-28 flex items-center justify-center">
                        {topCard ? (
                            ...
                            >
                                <div className={`w-full h-full rounded-lg ${getColorClass(topCard.color)} flex items-center justify-center text-white font-bold`}>
                                    {topCard.value}
                                </div>
                            </motion.div>
                        ) : (
