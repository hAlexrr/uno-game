...
export default function UnoCardComponent({ card, onClick, disabled = false }: UnoCardProps) {
  ...
  return (
    <motion.button
      className={cn(
        "w-16 h-24 rounded-lg shadow-md flex flex-col items-center justify-center font-bold relative", // Increased size
        getCardColor(),
    ...
    >
      ...
      <div className="text-2xl">{getCardSymbol()}</div> {/* Increased text size */}
